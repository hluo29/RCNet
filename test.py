...
if __name__ == '__main__':
    ...
    epoch_test_list = [450, 455, 460, 465, 470, 475, 480, 485, 490, 495, 500]
    for m in epoch_test_list:
        with torch.no_grad():
            model_net = Network().cuda()
            model_net.load_state_dict(torch.load('/home/cbl/LUOHAO/SASR/SASR_Pytorch/checkpoints/model_v2_corres_alpha085_maskavgp_att/%d_net_G.pth' % m))
            # model_v1_corres_alpha085_maskavgp
            logger.info('%d_net_G.pth load over' % m)
            value_psnr = 0
            value_ssim = 0
            model_net.eval()
